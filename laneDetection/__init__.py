...
def laneDetector(image, pathToSave=None):
    leftX, leftY, rightX, rightY, windowedImage = findLanePixels(image)
    if (os.environ['PYTHON_ENV'] == "debug" and pathToSave is not None):
        print("~~~~~")
        print(pathToSave)
        path = pathToSave.replace(".jpg", "-windowed.jpg")
        cv2.imwrite(path, windowedImage)
    leftFit, rightFit = fitPolynomial(leftX, leftY, rightX, rightY)
    laneImage, laneMask = drawLane(image, leftFit, rightFit)
    if (os.environ['PYTHON_ENV'] == 'debug' and pathToSave is not None):
        path = pathToSave.replace(".jpg", "-laneDrawn.jpg")
        cv2.imwrite(path, laneImage)
    return laneMask


def drawLane(image, leftFit, rightFit):
    print(image)
    h, w = image.shape
    plotY = np.linspace(0, h - 1, h)
    leftFitX = leftFit[0]*plotY**2 + leftFit[1]*plotY + leftFit[2]
    leftPoints = drawLine(leftFitX, plotY)
    rightFitX = rightFit[0]*plotY**2 + rightFit[1]*plotY + rightFit[2]
    rightPoints = drawLine(rightFitX, plotY)
    laneSurface = drawLanePoly(leftFitX, rightFitX, plotY)
...
def drawLanePoly(lineLeft, lineRight, plotY, width=50):
...
def drawLine(line, plotY, width=50):
    leftBorder = line - width//2
    rightBorder = line + width//2
    # We create the point on the left side of the stripe (polygon)
    leftPoints = np.array(list(zip(leftBorder, plotY)))
    # We create the points of the right side, we invert them to
    # close the polygon
    rightPoints = np.array(np.flipud(list(zip(rightBorder, plotY))))
...
def fitPolynomial(leftx, lefty, rightx, righty):
    # leftx, lefty, rightx, righty = findLanePixels(binary_warped)
...
def findLanePixels(binary_warped, nwindows = 10, margin = 100, minpix = 50, windowsColor=(0,255,255)):
